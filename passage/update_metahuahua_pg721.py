...
if "migrations" not in json_data:
    print("No migrations found in json data")
    sys.exit(0)

updated_list = []

for data in json_data['migrations']:
    token_id = data['token_id']
    if token_id in csv_data:
        if 'extension' not in data:
            continue

        data['extension']['image'] = csv_data[token_id]['IMAGE']
        data['extension']['name'] = csv_data[token_id]['NAME']

        if 'attributes' not in data['extension']:
            continue

        for attribute in data['extension']['attributes']:
...
with open("updated_pg721_migrations.json", 'w', encoding='utf-8') as json_file:
