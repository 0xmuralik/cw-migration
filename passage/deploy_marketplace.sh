...
MARKET_CONTRACT=$(passage tx wasm instantiate "$marketplace_code" "$MARKET_INIT" --from "$KEY" --label "marketplace v2" --admin "$minter_addr" --gas auto --gas-adjustment 1.15 -y -b block| jq -r '.logs[0]["events"][0]["attributes"][0]["value"]')
