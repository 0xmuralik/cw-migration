...
passage tx wasm store "$PATH_TO_CONTRACTS"/artifacts/marketplace_v2.wasm --from "$KEY" --gas auto --gas-adjustment 1.15 --chain-id "$CHAINID" -y -b block --keyring-backend "$KEYRING_BACKEND" --node "$NODE"
...
passage tx wasm instantiate "$CODE_ID" "$MARKET_INIT" --from "$KEY" --chain-id "$CHAINID" --label "marketplace v2" --admin "$minter_addr" --gas auto --gas-adjustment 1.15 -y -b block --keyring-backend "$KEYRING_BACKEND" --node "$NODE"

MARKET_CONTRACT=$(passage query wasm list-contract-by-code "$CODE_ID" --node "$NODE" --output json | jq -r '.contracts[-1]')
